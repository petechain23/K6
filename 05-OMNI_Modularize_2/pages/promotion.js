...
import { BASE_URL, outlet_depot, customTrendResponseTime, customTrendSuccessRate, customTrendRequestCount } from '../config.js';
...
export function promotions(cookies) {
  ...
  //Pick sequential outlet_external_id
  for (let i = 0; i < outlet_depot.length; i++) {
    ...
    // REP - After fix:
    const res = http.get(`${BASE_URL}/admin/promotions/get-list?distributor_external_id=302-BBI&outlet_external_id=${external_id}&start_before=2025-03-20T00:00:00.000Z&end_after=2025-03-20T00:00:00.000Z`, { headers: { cookies: cookies, 'Content-Type': 'application/json' } });
    
    // REP - Before fix:
    //const res = http.get(`${BASE_URL}/admin/promotions/get-list?depot_external_id=302-BBI-D03-MAIN&outlet_external_id=${external_id}&start_before=2025-03-20T00:00:00.000Z`, { headers: { cookies: cookies, 'Content-Type': 'application/json' } });
